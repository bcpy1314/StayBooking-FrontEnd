...
// export means the class maybe used by other file, similar to public
// every stay
export class StayDetailInfoButton extends React.Component {
  ...
  handleCancel = () => {
    // if people click, change the modal status
    this.setState({
      modalVisible: false,
    });
  };
  ...
  render() {
    ...
    return (
      <>
        ...
        {modalVisible && ( // if previous statement is true then operate the second statement
          <Modal
            title={name}
            centered={true}
...
class MyStays extends React.Component {
  state = {
    loading: false, // indicate is getting data
    data: [], // get data
  };
  ...
  render() {
    return (
      <List
        loading={this.state.loading}
        grid={{
          // how to adjust layout of grids to fit in different screen size
          gutter: 16, //
          xs: 1,
          sm: 3,
          md: 3,
        ...
        dataSource={this.state.data} // datasource: usually an array
        renderItem={(
          item // get an  item object
        ) => (
          <List.Item>
            <Card
              key={item.id} // assign an ID to each card so it can rearrange later if needed
              title={
...
class HostHomePage extends React.Component {
  render() {
    return (
      //defaulActiveKey: when user first come to browse the tab, define which tab is set as default
      //destroyInactiveTabPane: clean the content of inactive tabs
